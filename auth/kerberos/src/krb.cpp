    ...
    CF_logger& cf_logger )
{
    ...
    std::cerr << Util::getCurrentTime() << '\t'
              << "DEBUG: fetch_gmsa_password_and_create_krb_ticket ENTRY"
    ...
    if ( krb_ticket != NULL )
    {
        ...
        std::cerr << Util::getCurrentTime() << '\t'
                  << "DEBUG: gmsa_account_name=" << gmsa_account_name
    ...
    if ( !krb_cc_name.empty() )
    {
        ...
        {
            std::cerr << Util::getCurrentTime() << '\t'
                      << "DEBUG: Successfully set KRB5CCNAME=" << krb_cc_name << std::endl;
        }
        else
        {
            std::cerr << Util::getCurrentTime() << '\t'
                      << "DEBUG: Failed to set KRB5CCNAME=" << krb_cc_name << std::endl;
        }
    }
    ...
    std::string kinit_cmd = std::string( "dotnet " ) + std::string( install_path_for_decode_exe ) +
                            ...
                            " -V " + default_principal;
    std::cerr << Util::getCurrentTime() << '\t' << "INFO: kinit command: " << kinit_cmd
              << std::endl;
    ...
    if ( error_code == 0 )
    {
        ...
        std::cerr << Util::getCurrentTime() << '\t'
                  << "DEBUG: Post-kinit klist verification: " << klist_result.first
                  << " output: " << klist_result.second << std::endl;

        // Also check default cache to see what's there
        std::pair<int, std::string> klist_default = Util::exec_shell_cmd( "klist 2>&1" );
        std::cerr << Util::getCurrentTime() << '\t'
                  << "DEBUG: Post-kinit default cache: " << klist_default.first
                  << " output: " << klist_default.second << std::endl;
    }
...
bool is_ticket_ready_for_renewal( krb_ticket_info_t* krb_ticket_info, CF_logger& cf_logger )
{
    ...
    std::cerr << Util::getCurrentTime() << '\t'
              << "DEBUG: RENEWAL_CHECK klist result: error_code=" << krb_ticket_info_result.first
              << " output_length=" << krb_ticket_info_result.second.length() << std::endl;
    ...
    if ( krb_ticket_info_result.first != 0 )
    {
        ...
        std::cerr << Util::getCurrentTime() << '\t' << "ERROR: klist failed for command " << cmd
                  << std::endl;
        std::cerr << Util::getCurrentTime() << '\t'
                  << "DEBUG: RENEWAL_CHECK klist error output: " << krb_ticket_info_result.second
                  << std::endl;
        return false;
    }
